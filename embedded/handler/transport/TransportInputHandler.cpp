...
void TransportInputHandler::setupBindings() {
    api_.onPressed(ButtonID::BOTTOM_LEFT, [this]() { togglePlay(); });

    api_.onPressed(ButtonID::BOTTOM_RIGHT, [this]() { toggleRecord(); });

    api_.onPressed(ButtonID::BOTTOM_CENTER, [this]() { stop(); });
}
