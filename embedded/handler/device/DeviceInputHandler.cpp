...
void DeviceInputHandler::setupMacroBindings() {
    ...
    for (uint8_t i = 0; i < Device::PARAMETER_COUNT; i++) {
        // Encoder turn
        api_.onTurned(macroEncoders[i], [this, i](float value) { handleParameterChange(i, value); }, scope);

        // Button touch (press/release)
        api_.onPressed(macroButtons[i], [this, i]() { protocol_.send(Protocol::DeviceMacroTouchMessage{i, true}); }, scope);
        api_.onReleased(macroButtons[i], [this, i]() { protocol_.send(Protocol::DeviceMacroTouchMessage{i, false}); }, scope);
    }
}
...
void DeviceInputHandler::setupPageSelectorBindings() {
    ...
    api_.onTurnedWhilePressed(
        ...
        ButtonID::LEFT_BOTTOM,
        [this](float position) {
            if (pageSelection_.count == 0) return;
    ...
    api_.onReleased(ButtonID::LEFT_BOTTOM, [this]() { handlePageSelectorRelease(); }, scope);
}

void DeviceInputHandler::setupDeviceSelectorBindings() {
    lv_obj_t* scope = scope_;

    api_.onPressed(ButtonID::LEFT_CENTER, [this]() { handleDeviceSelectorRequest(); }, scope);

    api_.onTurnedWhilePressed(
        EncoderID::NAV,
        ButtonID::LEFT_CENTER,
        [this](float position) { handleDeviceSelectorNavigation(position); },
        scope);

    api_.onCombo(ButtonID::NAV, ButtonID::LEFT_CENTER, [this]() { handleDeviceSelectorEnter(); }, scope);

    api_.onReleased(ButtonID::LEFT_CENTER, [this]() { handleDeviceSelectorRelease(); }, scope);
