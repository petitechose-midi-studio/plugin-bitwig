...
struct DeviceChangeMessage {
    ...
    etl::string<STRING_MAX_LENGTH> deviceTrackName;
     ...
     */
    static constexpr uint16_t MAX_PAYLOAD_SIZE = 4815;

    /**
     * Minimum payload size in bytes (with empty strings)
     */
    static constexpr uint16_t MIN_PAYLOAD_SIZE = 415;
     ...
     */
    static etl::optional<DeviceChangeMessage> decode(
        ...
        // Decode fields
        etl::string<STRING_MAX_LENGTH> deviceTrackName;
        if (!decodeString<STRING_MAX_LENGTH>(ptr, remaining, deviceTrackName)) return etl::nullopt;
        etl::string<STRING_MAX_LENGTH> deviceName;
        ...
        return DeviceChangeMessage{deviceTrackName, deviceName, isEnabled, pageInfo_data, macros_data};
    }
