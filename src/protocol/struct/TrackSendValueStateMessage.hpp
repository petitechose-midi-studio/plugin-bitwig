...
struct TrackSendValueStateMessage {
    ...
    uint16_t encode(uint8_t* buffer, uint16_t bufferSize) const {
        ...
        encodeString(ptr, sendDisplayValue);
    ...
    static std::optional<TrackSendValueStateMessage> decode(
        ...
        std::string sendDisplayValue;
        if (!decodeString(ptr, remaining, sendDisplayValue)) return std::nullopt;
    ...
    const char* toString() const {
        ...
        ptr += snprintf(ptr, end - ptr, "# TrackSendValueState\ntrackSendValueState:\n");
