...
DeviceView::DeviceView(lv_obj_t *zone, const Config &viewConfig)
    : view_config_(viewConfig), initialized_(false), zone_(zone), top_bar_container_(nullptr),
      body_container_(nullptr) {
    for (auto &widget : widgets_) { widget = nullptr; }
...
DeviceView::~DeviceView() {
    for (auto &widget : widgets_) { widget.reset(); }
...
void DeviceView::sync() {
    if (!initialized_ || !state_.dirty.any()) return;

    if (state_.dirty.device) syncDeviceInfo();

    for (uint8_t i = 0; i < 8; i++) {
        if (state_.dirty.parameters[i]) syncParameter(i);
    }

    if (state_.dirty.pageSelector) syncPageSelector();

    if (state_.dirty.deviceSelector) syncDeviceSelector();

    if (state_.dirty.trackSelector) syncTrackSelector();

    state_.dirty.clear();
}

void DeviceView::syncDeviceInfo() {
    if (!top_bar_component_) return;

    top_bar_component_->render({.deviceName = state_.device.name.c_str(),
...
void DeviceView::syncParameter(uint8_t index) {
    ...
    if (param.metadataSet && !widgets_[index]) {
        switch (param.type) {
            case static_cast<uint8_t>(Device::ParameterType::BUTTON):
                widgets_[index] = std::make_unique<ParameterButtonWidget>(
                    body_container_, Layout::WIDGET_WIDTH, Layout::WIDGET_HEIGHT, index);
                break;
            case static_cast<uint8_t>(Device::ParameterType::LIST):
                widgets_[index] = std::make_unique<ParameterListWidget>(
                    body_container_, Layout::WIDGET_WIDTH, Layout::WIDGET_HEIGHT, index,
                    param.discreteCount);
                break;
            case static_cast<uint8_t>(Device::ParameterType::KNOB):
            default: {
                bool is_centered = (param.origin == 0.5f);
    ...
    if (!widgets_[index]) return;

    widgets_[index]->setName(param.name.c_str());
    widgets_[index]->setValueWithDisplay(param.value, param.displayValue.c_str());

    if (param.type == static_cast<uint8_t>(Device::ParameterType::BUTTON) ||
        param.type == static_cast<uint8_t>(Device::ParameterType::LIST)) {
        widgets_[index]->setDiscreteMetadata(param.discreteCount, param.discreteValueNames,
                                             param.currentValueIndex);
    }

    lv_obj_t *container = widgets_[index]->getContainer();
    if (container) {
        if (param.visible) lv_obj_clear_flag(container, LV_OBJ_FLAG_HIDDEN);
        else lv_obj_add_flag(container, LV_OBJ_FLAG_HIDDEN);
    }
}

void DeviceView::syncPageSelector() {
    if (!page_selector_) return;

    auto &ps = state_.pageSelector;
    page_selector_->render(
        {.names = &ps.names, .selectedIndex = ps.selectedIndex, .visible = ps.visible});
}
...
void DeviceView::syncDeviceSelector() {
    ...
    device_selector_->render({.names = &ds.names,
...
void DeviceView::syncTrackSelector() {
    if (!track_selector_) return;

    auto &ts = state_.trackSelector;
    track_selector_->render({.names = &ts.names,
...
void DeviceView::setParameterTypeAndMetadata(uint8_t paramIndex, uint8_t parameterType,
                                             int16_t discreteCount,
                                             const std::vector<std::string> &discreteValueNames,
                                             uint8_t currentValueIndex, float origin,
                                             const char *displayValue) {
    if (!initialized_ || paramIndex >= 8 || !body_container_) return;
...
void DeviceView::setParameterValueWithDisplay(uint8_t paramIndex, float normalizedValue,
                                              const char *displayValue) {
    if (!initialized_ || paramIndex >= 8) return;
...
void DeviceView::setParameterName(uint8_t paramIndex, const char *name) {
    if (!initialized_ || paramIndex >= 8 || !name) return;
...
void DeviceView::setupLayout() {
    ...
    for (int i = 0; i < 8; i++) {
        if (widgets_[i]) {
            ...
            lv_obj_set_grid_cell(widgets_[i]->getContainer(), LV_GRID_ALIGN_CENTER, col, 1,
                                 LV_GRID_ALIGN_CENTER, row, 1);
        }
    }
}
...
void DeviceView::createDeviceStateBar() {
    ...
    top_bar_component_->render({.deviceName = "Device 1",
                                .deviceEnabled = true,
...
void DeviceView::updateDeviceState(int displayIndex, bool enabled) {
    if (!device_selector_ || !device_selector_->isVisible()) return;

    // Update state so it persists across navigation
    if (displayIndex >= 0 &&
        displayIndex < static_cast<int>(state_.deviceSelector.deviceStates.size())) {
